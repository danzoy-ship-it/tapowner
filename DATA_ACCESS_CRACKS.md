...
3. The **{GUID} comes from the reports inventory** — the reliable listing endpoint is `GET https://prod-container.trueprodigyapi.com/public/config/reports` (bare host, NOT under /trueprodigy/cadpublic; `Authorization: <token>`; Origin = county TP site) → all rows with `reportS3ID = {slug}/{GUID}.{ext}`. The **.zip rows in category "Certified Exports" / "DATA EXPORTS" / "Appraisal Roll and Totals"** (pick the "…APPRAISAL ROLL/EXPORT" one, skip JSON/MINERALS/totals-PDF) are the whole-county PACS exports.

**REFINEMENTS (2026-07-15, verified Webb/Harrison/Cooke/Reeves/Limestone/ValVerde):**
- **Auth header varies by district: try BOTH `Bearer <token>` AND raw `<token>`.** Webb/Limestone/Cooke worked with Bearer; Harrison/Reeves need the RAW token (Bearer → HTTP 500). The old `/public/reportcategory/{cat}/search` path now 404s on many — use `/public/config/reports`.
- **The export zip is often NESTED:** outer zip = `{layout PDFs/xlsx} + {inner ..._Cert_Export_Files_*.zip}`; extract the inner zip, THEN feed its `.TXT` members to `load_pacs_roll.py`.
- **Per-district INFO width varies** (Harrison INFO = 9814 chars vs the standard ~9248): the IMPROVEMENT_DETAIL member (prop_id/type/sqft/year) parses fine, but APPRAISAL_INFO deed_dt/exemption/geo_id byte-offsets may be off → improvements+sqft load, deed/exempt may need re-derived offsets. Prop_id join still works.
- Which districts expose a .zip export vs PDF-only reports: **YES** Webb, Harrison, Cooke, Reeves, Limestone; **NO (all PDF)** Hunt, Anderson, Wharton, Hidalgo, JimWells, Maverick → those stay records-request.
...
- **Anderson (48001) year_built — re-verified 2026-07-16, NO free bulk FILE (confirmed, don't re-hunt):** downloaded the actual StratMap 2025 county shapefile (`tnris-data-warehouse.s3.us-east-1.amazonaws.com/LCD/collection/stratmap-2025-land-parcels/items/shp/stratmap-2025-land-parcels-anderson_48001_shp.zip`, 43,894 recs = exact CAD count) → `YEAR_BUILT` column present but **0/43,894 populated** (owner/situs/values ARE populated). CAD site = pure TP SPA (every path = 2966-byte "Public Portal" shell, old DNN GIS/shapefile page gone). TP `/public/config/reports` = 18 rows **all .pdf** (Certified Totals/Annual/Mass-Appraisal), NO data-export zip — the prior "PDF-only" verdict holds. TP search grid + CSV export result-columns = owner/value/situs/geo only (**no year_built column**). AGOL "Anderson County Parcel Data" (org `wallpope`, services7/yfE8xVBACewiUO4D) = **token-gated** private vendor; "Anderson Parcels" (timtation101) = actually 265 Grimes recs, empty. **year_built IS freely exposed per-property** via the TP improvement API `GET prod-container.trueprodigyapi.com/public/propertyaccount/{pAccountID}/improvement` → `actualYearBuilt`+`livingArea` (tested 8/8 improved accts, yrs 1970–2022) — this is Regrid's source (CAD CAMA, "Authoritative(County)"), but it's **1 call/acct = per-property** (off-limits for bulk mass-harvest; it IS the app-lane fill-on-blank source). Guardrail-clean bulk route = the **$0 PIA for the TP Appraisal Export** already queued in RECORDS_REQUESTS.md line 35. **Join key: TP `pid` == StratMap `Prop_ID` == `parcels.source_property_id`, with float artifact (`60110.00000000`→`60110`, use `--normalize-float`); GEO_ID matches TP geoID too.**

---

## ★ STATEWIDE BACKDOOR + the GIS≠CAMA lesson (2026-07-16 — the big one)

After wrongly concluding 106 counties' `year_built` was "gated," Frederick pushed: *Regrid HAS it, public data can't be sold, so a source EXISTS — FIND IT.* A Fable-5 workflow did. Two reusable lessons:

1. **GIS layer ≠ CAMA export.** `year_built` (and sqft/beds/roof) is NEVER in the free ArcGIS parcel FeatureServers — those carry owner/value/deed/geometry from the *mapping* vendor (BIS, etc.). It lives in the **CAMA/appraisal export** (improvement-detail table) from the *appraisal* vendor (PACS/Tyler, True Prodigy, SWData, P&A). StratMap's `YEAR_BUILT` column is 0%-populated for blind counties. **For building attributes, always target the CAMA/appraisal export, never the GIS parcel layer.** This one mistake caused every false "absent" verdict.

2. **The statewide backdoor: Texas Comptroller EARS.** All 254 CADs must submit their certified appraisal roll to PTAD annually in one standardized format (Tax Code 26.01(b)). The EARS Record Layout Manual **confirms `AJR23 Year Built`** + `AJR22` sqft + improvement value. **ONE Texas PIA to the Comptroller (open.records@cpa.texas.gov) = year_built for ALL 254 counties**, cost-of-reproduction only (files sit on privatesftp.cpa.texas.gov/incoming/ptad_ears; appraisal roll is public, not shielded by §552.149). This is how aggregators get statewide coverage — Regrid's per-county price is convenience markup on free public data. Request template + loader plan in `RECORDS_REQUESTS.md` (#1). Meta-lesson: **look for the STATE aggregation point before cracking N counties.**

**Aggregators' actual per-property harvest surfaces (FREE, proven live 2026-07-16; per-property → Frederick's mass-harvest ethics call applies):**
- **True Prodigy open API:** `POST prod-container.trueprodigyapi.com/trueprodigy/cadpublic/auth/token {"office":"<County>"}` → ~5-min JWT → `GET /public/propertyaccount/{pAccountID}/improvement` → `details[].actualYearBuilt`. Proven: Anderson, Jim Wells (fully verified: 2016/1982…), Maverick.
- **PACS/Tyler eSearch:** `esearch.{county}cad.org` Property-Detail/GetImprovements → Year Built + Eff. Year Built (Jackson R49010=2020, Matagorda, Rusk). Bulk = PACS `APPRAISAL_IMPROVEMENT_DETAIL.TXT` via per-CAD PIA → `data/load_pacs_impdetail_attributes.py`.
- The `sourceurl` field in Regrid is a per-PARCEL deep-link, NOT a bulk pointer (a red herring we chased).
