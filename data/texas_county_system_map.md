...
## 2026-07-16 — technical fixes on the 3 remaining loose ends (no new agents, direct engineering)
- **Erath 48143** — NOT actually broken: earlier "BIS FS oversized, fetch times out" was my OWN bash-tool timeout (3m20s) expiring mid-fetch on a 123,794-row layer (~5 deed-history rows/parcel via Deed_Seq), not a server or loader problem. Re-ran with room to breathe: 89s total. → sale 23,772 (96% of county, join apn==geo).
- **Shackelford 48417** — NEW `load_pacs_print_report.py`: parses the human-readable print-report roll (the only format this CAD posts) by regex-matching the per-property header line (`prop_id owner_id pct TYPE Geo:... Market:...`) + scanning forward for the `Exemptions:` line (robust to the variable Agent:/DBA:/blank line count between records). Filters to TYPE='R' (skips Personal/Mineral line-items, which is why the file has 17K+ blocks for a 5.5K-parcel county). Join apn==geo verified 85% + spot-checked (owner name match). → exemptions 962.
- **Rusk 48401** — RETRIED, still IP-locked (302→/Home/UserLockedOut). This is the server's own decision, not a transient blip — further retries would just be hammering their site. Partial data (3,303 exemptions via Wayback, verified) stands; revisit in a future session from a different connection.

## 2026-07-16 — AGGRESSIVE last-shot on the 12 records-request counties (2 fable-5 agents, "it's on the website / timeout-vs-rejection" mandate)
**Net: 1 new loadable increment (Smith ag-exemptions), 11 confirmed genuinely gated with fresh multi-angle proof.** No sale-PRICE column found anywhere (Hill stays the only one).
- **Smith 48423** — the old "smithcountymapsite.org → 500" was OUR error (idle ArcGIS pool cold-start, not a block). County GIS `smithcountymapsite.org/publicgis/rest/services/Gallery/TaxParcelQuery/MapServer/1` layer already gave sqft/year; NEW: `AG_Exemption='Y'` → **loaded 17,572 ag-exemption parcels** (exemptions ['AG'], join 'R'+PIN). Full CAMA (segments/beds/homestead/deed) still only behind the GSA `smithcad-search.gsacorp.io` session-POST export → records request.
- **Montague 48337** — CORRECTION: it's **Pritchard & Abbott**, NOT SWData. P&A `gisdata.pandai.com/pamaps02/rest/services/Montague/MontagueCADPublic/MapServer/0` = owner/value/legal/situs only. **Its P&A schema literally has `Adjusted_Price` (SALE PRICE) + `Total_Sqft` + Deed_Volume/Page/Is_Exempt/Homestead_Code — all present but NULLED on the public endpoint.** → precise records-request ask (the P&A export carrying those columns).
- **Hopkins 48223** — new anonymous FS `services8.arcgis.com/S14quRRuDSaYoUKA/Hopkins_County_Parcels/FeatureServer/0` (DiscoveryLandGroup republish, 25,155 rows) = Prop_ID/GEO_ID/OWNER/LAND+IMP+MKT value/situs. YEAR_BUILT=0, DATE_ACQ=uniform extract stamp (not a deed date). No signal field we track → NOT loaded.
- **PaloPinto 48363 / Floyd 48153 / Lipscomb 48295 / Motley 48345** — CONFIRMED gated: followed the SWData session token/302 the prior curl botched, read the full rendered menu (webIndex/webExemptions[schedule tables]/webPubs[PDF]/webDownloads[PDF forms]); every export route 404s; `/downloads/{DBKEY}/` = 403 (exists, listing denied); Wayback = zero data files ever. Per-property webProperty.aspx = vetoed. Motley has own MOTLEYCAD dbkey (not Floyd-combined).
- **Anderson 48001 / Wharton 48481** — CONFIRMED: TP `/public/config/reports` enumerated live = 18 / 29 reports, ALL PDF (+ 2 utility-bill xlsx for Wharton), zero appraisal .zip. Real wall.
- **Goliad 48175** — CONFIRMED: BIS org `services8.arcgis.com/WbC8UcChzGlcbEPR` subscription STILL disabled today (re-verified 3 ways: direct 499 Token Required, proxy 403 Subscription-disabled, webmap /data 403). Re-poll monthly = instant solve if re-enabled. pandai layer stripped (Market/Deed 0-populated).
- **RedRiver 48387** — CONFIRMED: BIS `services6.arcgis.com/QegVDgwL3C5CaiQS/RedRiverCADWebService/FeatureServer/0` deed/value fields 0/13,812 populated (deliberate strip). rrcad.org = CAGI CMS, per-property POST search only, every /Export /Download = 404.
- **Winkler 48495** — CONFIRMED: pandai layer stripped; eSearch DataRecords all PDF; old TP `/data/_uploaded/file/` dir now 404.
